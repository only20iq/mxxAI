...
function sent__s(messagecbbtbtnrte,noeval=false,imgxx=false){
  ...
  if(dataxxxx == "websocket" || dataxxxx == "close" || dataxxxx == "kapat" || dataxxxx == "durdur"){
    ...
    if(target_A=="__SERVER__NO_ENC__"){
      ...
      translate_Target_TR(dataxxxx,"tr").then(function(result) {
        ...
        var asdasdassd = ai_cevapla(result[0],false,1);
        ...
        translate(asdasdassd,"xxx",lang__x).then(function(resultxxx) {
          ...
          if(/<img|<iframe/.test(__cevap__) || __cevap__.length > 70){
            sent__s(__cevap__,noeval,"img");
          }else{
...
textarea.addEventListener("input", function() {
 ...
 result_0_data.forEach(function(obje) {
    ...
    const ratio1 = similarity (deger, aranan); // Levenshtein mesafesi ile

    const ratio2 = similarity_1 (deger, aranan); // Harf kesişimi ile
     
    // Benzeme oranlarının ortalamasını alın

    const oran = (ratio1 + ratio2) / 2;
