...
function quad_thread(text) {
  ...
  function ekler(e) {
    ...
    if (e.includes("+")) { // e değerinin içinde + olup olmadığını kontrol et
      ...
      let result = e+"/"+last+"ımı/"+last+"lar/"+last+"ler/"+last+"ları/"+last+"imi/"+last+"nı/"+last+"ni/"+last+"ı/"+last+"i/"+last+"mi/"+last+"ğı/"+last+"yi/"+last+"yı/"+last+"mı/"+last+"am/"+last+"dan/"+last+"den/"+last+"lük/"+last+"lik/"+last+"cilik/"+last+"ci/"+last+"ciyim/"+last+"çılık/"+last+"cülük/"+last+"rizm/"+last+"izm/"+last+"zm/"+last+"nın/"+last+"nin/"+last+"ten/"+last+"tım/"+last+"ın/"+last+"in/"+last+"ite/"+last+"ma/"+last+"tan"; // Sonucu oluştur
      ...
      return e+"/"+e+"ımı/"+e+"lar/"+e+"ler/"+e+"ları/"+e+"imi/"+e+"nı/"+e+"ni/"+e+"ı/"+e+"i/"+e+"mi/"+e+"ğı/"+e+"yi/"+e+"yı/"+e+"mı/"+e+"am/"+e+"dan/"+e+"den/"+e+"lük/"+e+"lik/"+e+"cilik/"+e+"ci/"+e+"ciyim/"+e+"çılık/"+e+"cülük/"+e+"rizm/"+e+"izm/"+e+"zm/"+e+"nın/"+e+"nin/"+e+"ten/"+e+"tım/"+e+"un/"+e+"ın/"+e+"in/"+e+"ite/"+e+"ma/"+e+"tan";
  ...
  for (let item of wait_dataset_cache["kötü"]) {
    ...
    var dataset = _cache_dataset_data+"+kötü/iğrenç/berbat/rezil/kalitesiz/beceriksiz/vasıfsız/gereksiz/rezil/berbat/yeteneksiz/nefret/sevmiyom/beğenmiyom/beğenmiyorum=this";
...
function ai_cevapla(metin,onlytext=false,random__create_func=1) {
  ...
  if(test.length==0){
    (()=>{
      var deger_ = metin.replaceAll(" ","");
      var enYuksekOran = 0.7;
      var enCokBenzeyen = false;
      result_A_single_key.forEach(function(obje) {
      var ratio1 = similarity (obje, deger_); // Levenshtein mesafesi ile
      var ratio2 = similarity_1 (obje, deger_); // Harf kesişimi ile
      var oran = (ratio1 + ratio2) / 2;
      if (oran > enYuksekOran) {
          enYuksekOran = oran;
          enCokBenzeyen = obje;
      }
      });
      if (enCokBenzeyen!=false) {
        // alert("Şunu demek istedi galiba:"+enCokBenzeyen);
        [test,_] = addTest(test,single_thread(enCokBenzeyen));
      }
    })();
  }
...
function single_getAllKeys(data){
  const results = [];
  for (let value in data.single) {
...
var result_A_single_key = single_getAllKeys(data);
